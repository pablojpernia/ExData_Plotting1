...
# 5.0 Creating plotted file    ----
        ...
        # Setting a layout to allow 4 graphs
        
        par(mfrow=c(2,2))
        
        # Constructing the 4 graphs by invoking different plot functions 
        
        with(tib_households_wrangled, 
        # 1st plotted graph
              {plot(tib_households_wrangled$Date_Time,
                    tib_households_wrangled$Global_active_power,
                    type  = "l",
                    xlab  = "",
        ...
        # 2nd plotted graph        
                plot(tib_households_wrangled$Date_Time,
                     tib_households_wrangled$Voltage,
                     type  = "l",
                     xlab  = "datetime",
                     ylab  = "Voltage")
        ...
        # 3rd plotted graph
                plot(tib_households_wrangled$Date_Time,
                     tib_households_wrangled$Sub_metering_1,
                     type  =  "n",
                     xlab  = "",
                     ylab  = "Energy Sub Metering")
                with(tib_households_wrangled,
                     lines(tib_households_wrangled$Date_Time, 
                           tib_households_wrangled$Sub_metering_1))
                with(tib_households_wrangled,
                     lines(tib_households_wrangled$Date_Time, 
                           tib_households_wrangled$Sub_metering_2,
                           col = "red"))
                with(tib_households_wrangled,
                     lines(tib_households_wrangled$Date_Time, 
                           tib_households_wrangled$Sub_metering_3,
                           col = "blue"))
                legend("topright",
                       lty = 1,
        ...
        # 4th plotted graph
                plot(tib_households_wrangled$Date_Time,
                     tib_households_wrangled$Global_reactive_power,
                     type  = "l",
                     xlab  = "datetime",
